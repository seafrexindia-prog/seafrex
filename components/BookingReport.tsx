...
export const BookingReport: React.FC<BookingReportProps> = ({ onBack, currentUser }) => {
  ...
  const loadData = () => {
    ...
    // Filter logic: User involved as Provider OR Client
  ...
  const applyFilters = () => {
    ...
    setTimeout(() => {
        const filtered = bookings.filter(b => {
            // Defensive coding: Ensure fields exist before calling toLowerCase
            const refMatch = (b.bookingRef || '').toLowerCase().includes(searchTerm.toLowerCase());
            const polMatch = (b.pol || '').toLowerCase().includes(searchTerm.toLowerCase());
            const podMatch = (b.pod || '').toLowerCase().includes(searchTerm.toLowerCase());
            const offerMatch = (b.offerNumber || '').toLowerCase().includes(searchTerm.toLowerCase());
            
            const searchMatch = refMatch || polMatch || podMatch || offerMatch;
    ...
    <div className="flex flex-col h-full w-full bg-ocean-950/80 overflow-hidden animate-fade-in-up relative">
      ...
      <div className="shrink-0 h-14 flex items-center justify-between px-6 bg-white/5 border-b border-white/10 backdrop-blur-sm">
        <div className="flex items-center space-x-3">
           ...
           <span className="font-semibold text-lg text-white">Booking Report</span>
        </div>
         ...
         <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-4 bg-white/5 p-4 rounded-2xl border border-white/10 shadow-lg">
                <div className="relative flex-1 max-w-sm">
                    ...
                    <input
                        type="text"
                        placeholder="Search Ref, Route, Offer..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full bg-ocean-900/50 border border-ocean-700 rounded-lg pl-10 pr-4 py-2 text-white text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                    />
                </div>
                ...
                <div className="flex items-center space-x-2 bg-ocean-950/50 rounded-lg p-1 border border-ocean-700">
                    {(['ALL', 'PENDING', 'ACTIVE', 'COMPLETED'] as const).map(s => (
                        <button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${statusFilter === s ? 'bg-indigo-600 text-white' : 'text-ocean-400 hover:text-white'}`}>{s}</button>
                    ))}
                </div>

                <button onClick={applyFilters} disabled={isLoading} className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2 rounded-lg font-bold shadow-lg flex items-center disabled:opacity-50">
               ...
               <div className="overflow-x-auto" style={{minHeight: '200px'}}>
                 <table className="w-full text-left">
                    <thead className="bg-white/5 text-xs uppercase text-ocean-300">
                       <tr>
                          <th className="px-6 py-4 font-semibold w-[20%]">Booking Ref / Date</th>
                          <th className="px-6 py-4 font-semibold w-[20%]">Route</th>
                          <th className="px-6 py-4 font-semibold w-[20%]">Carrier & Vessel</th>
                          <th className="px-6 py-4 font-semibold w-[15%]">Cargo Details</th>
                               ...
                               <React.Fragment key={b.id}>
                                   ...
                                   <tr className={`hover:bg-white/5 transition-colors cursor-pointer ${isExpanded ? 'bg-white/5' : ''}`} onClick={(e) => toggleRow(b.id, e)}>
                                       <td className="px-6 py-4 align-top">
                                           <div className="font-mono font-bold text-white text-sm">{b.bookingRef !== 'PENDING' ? b.bookingRef : 'PENDING'}</div>
                                           <div className="text-[10px] text-ocean-400 mt-1 flex items-center">
                                               <Calendar className="w-3 h-3 mr-1"/> {new Date(b.createdAt).toLocaleDateString()}
                                           </div>
                                           <div className="text-[10px] text-ocean-600 mt-0.5 font-mono">{b.id}</div>
                                       </td>
                                       <td className="px-6 py-4 align-top">
                                       ...
                                       <td className="px-6 py-4 align-top">
                                           <div className="font-medium text-white text-xs">{b.shippingLine || '-'}</div>
                                           <div className="text-[10px] text-ocean-400 mt-1 flex items-center">
                                               <Anchor className="w-3 h-3 mr-1"/> {b.vesselName}
                                           </div>
                                           <div className="text-[10px] text-ocean-500">Voy: {b.voyage}</div>
                                       </td>
                                       <td className="px-6 py-4 align-top">
                                       ...
                                       <td className="px-6 py-4 text-right align-top relative">
                                           <div className="flex flex-col items-end gap-2 action-menu-container" onClick={e => e.stopPropagation()}>
                                               <div className="relative">
                                                    ...
                                                    {openActionId === b.id && (
                                                        <div className="absolute right-0 top-full mt-1 w-36 bg-ocean-900 border border-ocean-700 rounded-xl shadow-xl z-50 overflow-hidden animate-fade-in-up">
                                                            <button onClick={(e) => { e.stopPropagation(); toggleRow(b.id, e); setOpenActionId(null); }} className="w-full text-left px-3 py-2 text-xs text-cyan-400 hover:bg-white/10 flex items-center">
                                                                <Eye className="w-3 h-3 mr-2" /> View Details
                                                            </button>
                                                            <button onClick={(e) => { e.stopPropagation(); handlePrint(b); }} className="w-full text-left px-3 py-2 text-xs text-ocean-300 hover:bg-white/10 flex items-center">
                                                                <Printer className="w-3 h-3 mr-2" /> Print Report
                                                            </button>
                                                            <button onClick={(e) => { e.stopPropagation(); handleEmail(b); }} className="w-full text-left px-3 py-2 text-xs text-ocean-300 hover:bg-white/10 flex items-center">
                                                                <Mail className="w-3 h-3 mr-2" /> Email Status
                                                            </button>
                                                        </div>
                                                    )}
                                               </div>
                                               <button onClick={(e) => toggleRow(b.id, e)} className="text-xs flex items-center text-indigo-400 hover:text-white mt-1">
                                                  {isExpanded ? 'Collapse' : 'Expand'} {isExpanded ? <ChevronUp className="w-3 h-3 ml-1"/> : <ChevronDown className="w-3 h-3 ml-1"/>}
                                               </button>
                                           </div>
                                       </td>
                                   </tr>
                                       ...
                                       <td colSpan={6} className="px-6 py-4">
                                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
                                             ...
                                             <div className="col-span-1 border-r border-white/5 pr-4">
                                                <p className="text-ocean-500 font-bold uppercase mb-3">Partner Information</p>
                                                <div className="space-y-2">
                                                    <div className="flex justify-between">
                                                        <span className="text-ocean-400">Offer Reference:</span>
                                                        <span className="text-white font-mono">{b.offerNumber}</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className="text-ocean-400">Client:</span>
                                                        <span className="text-white">{b.clientName}</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className="text-ocean-400">Provider:</span>
                                                        <span className="text-white">{b.providerUser}</span>
                                                    </div>
                                                </div>
                                             </div>
                                             ...
                                             <div className="col-span-2">
                                                <p className="text-ocean-500 font-bold uppercase mb-3">Recent Status History</p>
                                                <div className="space-y-3 relative before:absolute before:inset-0 before:ml-1.5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-emerald-500/50 before:to-transparent pl-4">
                                                    {b.timeline.slice().reverse().slice(0, 3).map((log, idx) => (
                                                        <div key={idx} className="relative">
                                                            <div className={`absolute -left-4 top-1 w-3 h-3 rounded-full border border-ocean-950 ${idx === 0 ? 'bg-emerald-500' : 'bg-ocean-700'}`}></div>
                                                            <div className="flex justify-between items-start">
                                                                <div>
                                                                    <span className={`font-bold ${idx === 0 ? 'text-white' : 'text-ocean-400'}`}>{log.status.replace(/_/g, ' ')}</span>
                                                                    {log.remarks && <p className="text-ocean-500 italic mt-0.5">{log.remarks}</p>}
                                                                </div>
                                                                <div className="text-right">
                                                                    <div className="text-ocean-400">{new Date(log.date).toLocaleDateString()}</div>
                                                                    <div className="text-ocean-600 text-[10px]">{new Date(log.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ))}
